...
- `Home.tsx` and `home/`: `Intro` (namecard, 1952 → Today), `Hero`, `WhatGetsLost`, `ThreeCalls` (teal, pinned), `EveryVoice`, `StillHappening` (pinned sideways timeline), `TheBook`, `StepOne`.
- Routes: `HowItWorks.tsx` (three acts, complete annotated calls), `Families.tsx` (five reasons), `OurStory.tsx` (founder essay, verbatim), `Pricing.tsx`, `Start.tsx`, `Care.tsx`, `Organizations.tsx`, `Questions.tsx`, `Guides.tsx`, `Guide.tsx`, `Thanks.tsx`, `NotFound.tsx`, `LegalLayout.tsx` + `legalPrimitives.ts`.
