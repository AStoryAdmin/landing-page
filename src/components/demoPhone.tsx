...
const DemoPhone = ({
    ...
    const [stay, setStay] = useState<{ method?: string; why: string } | null>(
        null,
    );
    ...
    useEffect(() => {
        if (opened) phoneBodyRef.current?.focus({ preventScroll: true });
    }, [opened]);
    ...
    async function runDemo() {
        ...
        for (const turn of scenario.script) {
            ...
            if (turn.role === 'ai') {
                ...
                setStay(
                    turn.stay ? { method: turn.method, why: turn.stay } : null,
                );
                ...
                setIsTyping(true);
                await pause(Math.min(turn.text.length * 16, 2000));
                if (dead()) return standDown();
                setIsTyping(false);
        ...
        /* The reader chooses the next layer; never switch away while reading. */
        await pause(400);
        if (dead()) return standDown();
        setShowClose(true);
    ...
    return (
        <PhoneContainer>
            ...
            <PhoneStage>
                ...
                <Cover $gone={opened} aria-hidden={opened} inert={opened}>
                    <CoverChapter>{scenario.chapter}</CoverChapter>
                    <CoverQuote>
                        &ldquo;{scenario.cover.quote}&rdquo;
                    </CoverQuote>
                    <CoverAttribution>
                        {scenario.cover.attribution}
                    </CoverAttribution>
                ...
                <PhoneBody ref={phoneBodyRef} tabIndex={-1}>
                    ...
                    >
                        ...
                        <StayNote $show={Boolean(stay)}>
                            {stay ? (
                                <>
                                    {stay.method && (
                                        <MethodTag>{stay.method}</MethodTag>
                                    )}
                                    {stay.why}
                                </>
                            ) : null}
                    ...
                    <ChatMsgs
                        role="log"
                        aria-live="polite"
                        aria-label={`Conversation demo: ${scenario.label}`}
                    >
                        {messages.map((m) => (
                            <Fragment key={m.id}>
                                {m.note && (
                    ...
                    {opened && (
                        <ChatAction>
                            <ChatPlayButton
                                onClick={runDemo}
                                disabled={isPlaying}
                                aria-label={`Replay the ${scenario.label} conversation`}
                            >
                    ...
                    {showMemoryCard && (
                        <MemoryCardReveal
                            ref={memoryCardRef}
                            $show={memoryCardVisible}
                            aria-label="Memory saved from this conversation"
                        >
                            ...
                            <MemoryCard>
                                ...
                                <McLayers
                                    role="tablist"
                                    aria-label="What is kept from this conversation"
                                >
                                    {LAYERS.map((l) => (
                                        <McLayerTab
                                            key={l.id}
                                            ...
                                            onKeyDown={(event) => {
                                                ...
                                                pickLayer(LAYERS[next].id);
                                                document
                                                    .getElementById(
                                                        `mc-tab-${scenario.id}-${LAYERS[next].id}`,
                                                    )
                                                    ?.focus();
                                            }}
                                        >
                                ...
                                <McLayersHint $show>
                                    One conversation, kept three ways. Tap any
                                    of the three.
                                </McLayersHint>
                                ...
                                {layer === 'summary' && (
                                    <McPanel
                                        role="tabpanel"
                                        id={`mc-panel-${scenario.id}-summary`}
                                        aria-labelledby={`mc-tab-${scenario.id}-summary`}
                                    >
                                        <McSummary>
                                            {scenario.summary}
                                        </McSummary>
                                        <McExcerpt>
                                            &ldquo;{scenario.excerpt}&rdquo;
                                        </McExcerpt>
                                    </McPanel>
                                )}
                                ...
                                {layer === 'transcript' && (
                                    <McPanel
                                        role="tabpanel"
                                        id={`mc-panel-${scenario.id}-transcript`}
                                        aria-labelledby={`mc-tab-${scenario.id}-transcript`}
                                    >
                                        <McTranscript>
                                        ...
                                        <McVoiceNote>
                                            Word for word, nothing edited out,
                                            and searchable &mdash; the summary
                                            above is a layer over this, never a
                                            replacement for it.
                                        </McVoiceNote>
                                    </McPanel>
                                )}
                                ...
                                {layer === 'voice' && (
                                    <McPanel
                                        role="tabpanel"
                                        id={`mc-panel-${scenario.id}-voice`}
                                        aria-labelledby={`mc-tab-${scenario.id}-voice`}
                                    >
                                        <McClip>
                                            <WaveIcon />
                    ...
                    {showClose && (
                        <DemoClose>
                            <DemoCloseInner>
                                <DemoCloseTitle>
                                    Nobody has asked {scenario.teller} that
                                    before.
                                </DemoCloseTitle>
                                <DemoCloseSub>
                                    There is somebody in your family with an
                                    answer like that one, and no particular
                                    reason to ever say it out loud.
                                </DemoCloseSub>
                                <DemoCloseLink to={WAITLIST_HREF}>
                                    {WAITLIST_LABEL}
                                </DemoCloseLink>
                            </DemoCloseInner>
                        </DemoClose>
