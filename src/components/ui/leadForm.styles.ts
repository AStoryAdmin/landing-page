 ...
 *
 * Two grounds, because it appears on both: `$onDark` for the dark panels and
...
export const Field = styled.label<{ $onDark?: boolean }>`
    ...
    textarea {
        ...
        background: ${({ $onDark }) =>
            $onDark ? 'rgba(255,255,255,0.06)' : color.paperPure};
        border: 1px solid
            ${({ $onDark }) =>
                $onDark ? color.onDarkMuted : color.controlBorder};
        border-radius: 2px;
        transition:
    ...
    textarea::placeholder {
        color: ${({ $onDark }) =>
            $onDark ? 'rgba(243,235,221,0.45)' : color.faint};
    }

    input:focus-visible,
    textarea:focus-visible {
        border-color: ${({ $onDark }) =>
            $onDark ? color.onDark : color.primary};
        box-shadow: 0 0 0 3px
            ${({ $onDark }) =>
                $onDark ? 'rgba(224,160,63,0.28)' : 'rgba(184,81,38,0.18)'};
    }
...
export const Submit = styled.button<{ $onDark?: boolean }>`
    ...
    font-size: 1.0625rem;
    font-weight: ${weight.semibold};
    color: ${({ $onDark }) => ($onDark ? color.primaryDeep : color.onDark)};
    background: ${({ $onDark }) => ($onDark ? color.gold : color.primary)};
    border: none;
    border-radius: 2px;
    box-shadow: none;
    cursor: pointer;
...
export const ErrorNote = styled.p<{ $onDark?: boolean }>`
    ...
    border-radius: 2px;
    background: ${({ $onDark }) =>
        $onDark ? 'rgba(233,160,138,0.14)' : color.accentWash};
    font-size: ${type.sm};
    line-height: ${leading.normal};
...
export const Done = styled.div<{ $onDark?: boolean }>`
    ...
    background: ${({ $onDark }) =>
        $onDark ? 'rgba(224,160,63,0.1)' : color.paperPure};
    border: 1px solid
    ...
    strong {
        color: ${({ $onDark }) =>
            $onDark ? color.goldText : color.accentText};
    }
`;
