...
export type Tone = 'ivory' | 'paper' | 'wash' | 'deep' | 'teal' | 'gold' | 'ivoryDeep';

const tones: Record<Tone, ReturnType<typeof css>> = {
    ivory: css`background: ${color.ivory}; color: ${color.body};`,
    ivoryDeep: css`background: ${color.ivoryDeep}; color: ${color.body};`,
    paper: css`background: ${color.paper}; color: ${color.body};`,
    wash: css`background: ${color.primaryWash}; color: ${color.body};`,
    deep: css`
...
export const Section = styled.section<{ $tone?: Tone; $tight?: boolean; $flush?: boolean }>`
    ${({ $tone = 'ivory' }) => tones[$tone]};
    padding-block: ${({ $tight, $flush }) => ($flush ? '0' : $tight ? space.section : space.sectionLg)};
...
export const Container = styled.div<{ $wide?: boolean; $narrow?: boolean }>`
    width: 100%;
    margin-inline: auto;
    max-width: ${({ $wide, $narrow }) => ($wide ? layout.maxWidthWide : $narrow ? '780px' : layout.maxWidth)};
`;

/* ── Type ─────────────────────────────────────────────────────────────── */

export const Eyebrow = styled.p<{ $tone?: 'accent' | 'gold' | 'teal' | 'muted' }>`
    ...
    color: ${({ $tone = 'accent' }) =>
        $tone === 'gold' ? color.goldText : $tone === 'teal' ? color.primary : $tone === 'muted' ? color.faint : color.accentText};
`;
...
export const Italic = styled.em<{ $tone?: 'accent' | 'gold' }>`
    font-style: italic;
    color: ${({ $tone = 'accent' }) => ($tone === 'gold' ? color.goldText : color.accent)};
`;
...
const buttonBase = css`
    ...
    transition: background ${motion.base}, color ${motion.base}, border-color ${motion.base},
        transform ${motion.fast}, box-shadow ${motion.base};
...
export const buttonVariants = {
    primary: css`
        ...
        &:hover { background: ${color.accentHover}; box-shadow: ${shadow.lg}; }
    `,
    teal: css`
        background: ${color.primary};
        color: ${color.onDark};
    ...
    gold: css`
        ...
        box-shadow: ${shadow.md};
        &:hover { background: ${color.goldDeep}; }
    `,
    outline: css`
        background: transparent;
        color: ${color.primary};
        border-color: ${color.primaryLineStrong};
        &:hover { background: ${color.primaryWash}; border-color: ${color.primary}; }
    `,
    onDark: css`
        background: transparent;
        color: ${color.onDark};
        border-color: ${color.onDarkLine};
        &:hover { background: rgba(243, 235, 221, 0.1); border-color: ${color.onDark}; }
    `,
    ghost: css`
        background: transparent;
        ...
        min-height: 0;
        border-radius: 0;
        &:hover { color: ${color.accent}; transform: none; gap: 12px; }
    `,
} as const;
...
export const Card = styled.div<{ $onDark?: boolean; $raised?: boolean }>`
    background: ${({ $onDark }) => ($onDark ? 'rgba(243, 235, 221, 0.05)' : color.paperPure)};
...
export const Grid = styled.div<{ $cols?: number; $gap?: string; $min?: string }>`
    display: grid;
    gap: ${({ $gap }) => $gap ?? space.lg};
    grid-template-columns: ${({ $min = '260px' }) => `repeat(auto-fit, minmax(min(${$min}, 100%), 1fr))`};
...
export const Split = styled.div<{ $ratio?: string; $gap?: string; $align?: string }>`
    display: grid;
...
export const Divider = styled.hr<{ $onDark?: boolean }>`
    border: none;
    border-top: 1px solid ${({ $onDark }) => ($onDark ? color.onDarkLine : color.primaryLine)};
...
export const Badge = styled.span<{ $onDark?: boolean }>`
    ...
    padding: 8px 16px;
    border-radius: ${radius.pill};
    background: ${({ $onDark }) => ($onDark ? 'rgba(243, 235, 221, 0.10)' : color.paperPure)};
    border: 1px solid ${({ $onDark }) => ($onDark ? color.onDarkLine : color.primaryLine)};
...
export const StatNum = styled.div<{ $tone?: 'gold' | 'accent' | 'teal' }>`
    font-family: ${font.display};
    font-size: ${type.d3};
    font-weight: ${weight.regular};
    line-height: 1;
    letter-spacing: ${tracking.display};
    color: ${({ $tone = 'gold' }) =>
        $tone === 'accent' ? color.accent : $tone === 'teal' ? color.primary : color.goldText};
    margin-bottom: ${space.sm};
`;
...
export const Arrow = styled.span`
    ...
    a:hover &, button:hover & { transform: translateX(4px); }
`;
...
export const TextLink = styled(Link)`
    ...
    color: ${color.accentText};
    text-decoration: none;
    transition: color ${motion.fast}, gap ${motion.fast};
    &:hover { color: ${color.accent}; gap: 12px; }
`;
