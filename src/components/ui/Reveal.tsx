...
const Wrapper = styled.div<{ $visible: boolean; $delay: number; $shift: number }>`
    opacity: ${({ $visible }) => ($visible ? 1 : 0)};
    transform: translateY(${({ $visible, $shift }) => ($visible ? 0 : $shift)}px);
    transition: opacity 700ms cubic-bezier(0.22, 0.61, 0.36, 1) ${({ $delay }) => $delay}ms,
        transform 700ms cubic-bezier(0.22, 0.61, 0.36, 1) ${({ $delay }) => $delay}ms;
...
const Reveal = ({ children, delay = 0, shift = 18, className }: Props) => {
    ...
    return (
        <Wrapper ref={ref} className={className} $visible={visible} $delay={delay} $shift={shift}>
            {children}
        </Wrapper>
    );
};
