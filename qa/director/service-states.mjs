...
const browser=await chromium.launch({channel:"msedge",headless:true});const fixture='http://127.0.0.1:5181';const results=[];const check=async(name,fn)=>{try{await fn();results.push({name,passed:true});console.log('PASS',name);}catch(e){results.push({name,passed:false,error:e.message});console.log('FAIL',name,e.message);}};
...
await safe.close();await browser.close();await fs.writeFile('qa/director/service-states.json',JSON.stringify(results,null,2));if(results.some(x=>!x.passed))process.exitCode=1;